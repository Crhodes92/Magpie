...
import { Camera, CheckCircle, XCircle, Loader2, AlertTriangle, ExternalLink, ArrowRight, ArrowLeft } from 'lucide-react'
...
export default function BatchScoutPage() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const newItem: QueueItem = {
      ...
      phase: 'identifying',
      result: null, errorMsg: null, decision: null, savedItemId: null,
    }
  ...
  async function decide(id: string, decision: 'buy' | 'pass') {
    ...
    const body: Record<string, unknown> = {
      ...
      lane: item.result.lane, title: item.result.title, category: item.result.category,
      brand: item.result.brand ?? null, model: item.result.model ?? null,
      condition_note: item.result.condition_note ?? null,
      ai_identification: item.result, ai_confidence: item.result.confidence,
      est_value_low: item.result.est_value_low ?? null, est_value_high: item.result.est_value_high ?? null,
      max_bid: item.result.confidence >= 0.6 && item.result.est_value_low
        ? Math.round(item.result.est_value_low * 0.4 * 100) / 100 : null,
    }
    if (item.result.lane === 'card' && item.result.card_details) body.card_details = item.result.card_details
    ...
    setQueue(prev => {
      ...
      if (undecided.length > 0) setFocusedId(undecided[0].id)
      else if (!prev.some(i => i.id !== id && i.phase === 'identifying')) setFocusedId(null)
      return prev
    })
  }
            ...
            <div className="space-y-2 mb-6">
              ...
              {bought.map(item => (
                <div key={item.id} className="flex items-center gap-3 bg-white border-2 border-black rounded-xl p-3 shadow-[3px_3px_0_0_#000]">
                  ...
                  {item.savedItemId && (
                    <Link href={`/intake/${item.savedItemId}`} className="shrink-0 flex items-center gap-1 text-yellow-600 text-xs font-bold hover:text-black transition-colors">
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div className="flex items-center justify-between flex-1">
            ...
            {queue.length > 0 && <span className="text-gray-400 text-sm font-medium">{decided.length}/{queue.length} done</span>}
          </div>
        </div>
        <p className="text-gray-500 text-sm mb-5 pl-8">Tap to add photos · decisions queue up automatically</p>
        ...
        <label className="block cursor-pointer mb-5">
          ...
          <div className="flex items-center justify-center gap-3 bg-white border-2 border-dashed border-black rounded-2xl py-5 hover:bg-yellow-50 transition-colors">
            ...
            <span className="text-black font-bold text-sm">{queue.length === 0 ? 'Take first photo' : 'Add another photo'}</span>
          </div>
        </label>
              ...
              {focused.phase === 'ready' && focused.result && (
                <>
                  ...
                  {focused.result.category && (
                    <p className="text-gray-500 text-sm">{focused.result.category}{focused.result.brand ? ` · ${focused.result.brand}` : ''}</p>
                  )}
                  {focused.result.condition_note && <p className="text-gray-500 text-sm italic">{focused.result.condition_note}</p>}
                  ...
                  {(focused.result.est_value_low != null || focused.result.est_value_high != null) && (
                    <div className="bg-yellow-50 border-2 border-black rounded-lg px-3 py-2">
                      ...
                      <p className="text-black font-black">${focused.result.est_value_low ?? '?'} – ${focused.result.est_value_high ?? '?'}</p>
                      {focused.result.confidence >= 0.6 && focused.result.est_value_low != null && (
                        <p className="text-yellow-600 text-sm font-bold mt-0.5">Max bid: ${(focused.result.est_value_low * 0.4).toFixed(2)}</p>
                      )}
                    </div>
                  )}
                  <a
